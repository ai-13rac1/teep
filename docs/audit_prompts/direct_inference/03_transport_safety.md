...
Audit transport-layer request construction safety, bounded-resource handling, sensitive-data hygiene, and connection lifecycle management in direct inference proxy paths.
...
- that `Connection: close` is set on outbound requests to the attested server,
- that connection read/write timeouts are set and reasonable (preventing indefinite hangs against a malicious or slow server),
...
For direct inference providers that bypass standard Go `http.Client` TLS handling:
...
3. connection lifecycle safety assessment (close-after-use, timeout, reuse prevention),
